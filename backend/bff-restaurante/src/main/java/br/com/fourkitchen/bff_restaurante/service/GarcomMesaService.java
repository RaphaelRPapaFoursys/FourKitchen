...
public class GarcomMesaService {
    ...
    public void cancelarPedidoAntesDoPreparo(Integer idMesa, Integer idPedido, Authentication authentication) {
        if (idPedido == null || idPedido <= 0) {
            throw new BaseException(ErrorEnum.DADOS_INVALIDOS);
        }
        ...
        if (!"ENVIADO_COZINHA".equals(pedido.status())) {
            throw new BaseException(ErrorEnum.TRANSICAO_STATUS_INVALIDA);
        }

        cancelarPedidoNoMsPedidos(idPedido);
    }
    ...
    private void cancelarPedidoNoMsPedidos(Integer idPedido) {
        try {
            pedidoClient.cancelarPedidoAntesDoPreparo(idPedido);
        } catch (FeignException e) {
            if (e.status() == 404) {
                throw new BaseException(ErrorEnum.PEDIDO_NAO_ENCONTRADO);
            }
